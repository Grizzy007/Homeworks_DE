...
with all_dates as (
    ...
    union all select cast(merged_at as date) from {{ ref('pull_requests') }}
    union all select cast(opened_at as date) from {{ ref('issues') }}
    union all select cast(closed_at as date) from {{ ref('issues') }}
),
bounds as (
    select min(d) as min_d, max(d) as max_d from all_dates
),
calendar as (
