...
enriched as (
    select
        commit_sha, repo_name, pushed_by, branch, author_name, author_email, message,
        is_distinct, pushed_at,
...
dedup as (
    select *, row_number() over (partition by commit_sha order by pushed_at, event_id) as rn
    from enriched
)
select commit_sha, repo_name, pushed_by, branch, author_name, author_email, message,
       is_distinct, pushed_at, is_merge_commit, message_subject, message_length
from dedup
where rn = 1
