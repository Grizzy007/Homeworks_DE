...
ranked as (
    select *,
        row_number() over (partition by repo_name, pr_number
                           order by last_event_at desc, event_id desc) as rn
    from flat
)
select
    repo_name, pr_number, title, author_login, state, is_merged, is_draft,
