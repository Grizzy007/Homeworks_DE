...
flat as (
    select
        ...
        event_type, event_at, event_id
    from iss
    where p.issue.number is not null
),
agg as (
    select *,
        row_number() over (partition by repo_name, issue_number
                           order by event_at desc, event_id desc) as rn,
        sum(case when event_type = 'IssueCommentEvent' then 1 else 0 end)
            over (partition by repo_name, issue_number) as comment_events_seen
    from flat
)
select
    repo_name, issue_number, title, author_login, state,
